...
from tagger.train import run_epochs
from tagger.test import test
from tagger.dataset import Dataset
from tagger.model import NewEventTagger

from tagger.baseline_test import baseline_test
...
if __name__ == "__main__":
    ...
    method = args.method
    model_path = os.path.join("./save", "tagger."+args.model_name)
    ...
    if method not in rule_based_method_list:
        def init_model():
            ...
            return NewEventTagger(hparams, bert_model, dataset.label_weight).cuda()
    ...
    if hparams["rule_based"] == True:
        baseline_test(dataset, method, pasted_only=hparams["pasted_only"])
    else:
        if train:
